...
add_user() {
    ...
    if [ -f "$PUBLIC_KEY_FILE" ] && [ -f "$CONFIG_FILE" ]; then
        ...
        if [ -n "${SSH_SUBDOMAIN:-}" ] && [ -n "${PUBLIC_KEY_FILE:-}" ] && [ -f "$PUBLIC_KEY_FILE" ]; then
            local ssh_short_name
            ssh_short_name=$(echo "$SSH_SUBDOMAIN" | awk -F. '{if(NF>=2) print $(NF-1); else print $1}')
            local dnstt_ssh_data="16|dnstt_ssh|${ssh_short_name}-ssh|${SSH_SUBDOMAIN}|${default_resolver}|0|5000|bbr|1080|127.0.0.1|0|${pubkey}|${username}|${password}|1|${username}|${password}|22|0|127.0.0.1|0||udp|password|||0|443||||0||0|0|"
            local noizdns_ssh_data="16|sayedns_ssh|${ssh_short_name}-ssh|${SSH_SUBDOMAIN}|${default_resolver}|0|5000|bbr|1080|127.0.0.1|0|${pubkey}|${username}|${password}|1|${username}|${password}|22|0|127.0.0.1|0||udp|password|||0|443||||0||0|0|"
            echo ""
            echo -e "  ${CYAN}DNSTT+SSH:${NC}"
            echo -e "  ${WHITE}slipnet://$(echo -n "$dnstt_ssh_data" | base64 -w0)${NC}"
            echo ""
